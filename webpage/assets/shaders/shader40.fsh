// shader type: gen // shader written by unknown on glslsandbox.com  // shader translated to gles by Erogenous-Tones.com //----------------------------------------// shader name length of 40 characters
...
uniform int tex_l;                      // number of loaded texures  //f0:horz rotate:
//f1:vert rotate:
//f2:brightness:
...
vec2 res = vec2(tres.x/tres.y,1.0);
vec2 p = ( gl_FragCoord.xy / tres.y ) -(res/2.0);

vec2 m = (vec2(f0,f1)-0.5)*pi*vec2(2.,1.);
...
color_out = vec3(1.-abs(snoise(pos*4.0+vec3(0,time,0))));
color_out *= pos.y/f2+0.75;
color_out *= mix(vec3(0,0.5,1),vec3(0.,1,.5),pos.y);
