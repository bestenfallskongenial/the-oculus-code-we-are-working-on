...
#define LOG_NAME "VCSM"
                ...
                :	m_Screen (m_Options.GetWidth (), m_Options.GetHeight ()),
	                ...
	                m_MCP300X (&m_SPIMaster, ADC_VOLT_REF, SPI_CHIP_SELECT, SPI_CLOCK_SPEED),
...
TShutdownMode   CKernel::Run(void)
{
            m_Timer.MsDelay(500);
                ...
                if( filesystem_mount("emmc1-1", 
                    ...
                    gfx_init_vshaders(&state, filecounter[oldcount][vsh] /* g_loaded_vsh_old */, filecounter[newcount][vsh] /* g_loaded_vsh_new */);
                    // here we need the Overlay Menu shader init
                    gfx_init_fshaders(&state, filecounter[oldcount][fsh] /* g_loaded_fsh_old */, filecounter[newcount][fsh] /* g_loaded_fsh_new */);
                    // here we need the Overlay Menu program init
                    gfx_init_programs(&state, filecounter[oldcount][fsh] /* g_loaded_fsh_old */, filecounter[newcount][fsh] /* g_loaded_fsh_new */);
                    // here we need the Overlay Menu uniform init        
                    gfx_init_uniforms(&state, filecounter[oldcount][fsh] /* g_loaded_fsh_old */, filecounter[newcount][fsh] /* g_loaded_fsh_new */);
                    // here we need the Overlay Menu texture init
                    gfx_init_textures(&state, filecounter[oldcount][tex] /* g_loaded_tex_old */, filecounter[oldcount][tex] /* g_loaded_tex_new */); // we commented out the texture loading for the test of the h264 decoder to texture mechanism, but we need to load the textures at least one time for the brandlogo and the ui, so we load them here from the emmc, but we will not load them again from the usb later, so that we can test the h264 decoder to texture mechanism without interference of the usb loading

                    m_Watchdog.Start(TIMEOUT);

                    m_Timer.MsDelay(100);
 
                    filecounter[oldcount][vsh] /* g_loaded_vsh_old */ = filecounter[newcount][vsh] /* g_loaded_vsh_new */;    // also 0 at bytesize calc beacuse i loaded only here
                    filecounter[oldcount][omf] /* g_loaded_fsh_old */ = filecounter[newcount][omf] /* g_loaded_fsh_new */;
                    filecounter[oldcount][fsh] /* g_loaded_fsh_old */ = filecounter[newcount][fsh] /* g_loaded_fsh_new */;
                    filecounter[oldcount][omt] /* g_loaded_tex_old */ = filecounter[newcount][omt] /* g_loaded_tex_new */;                    
                    filecounter[oldcount][tex] /* g_loaded_tex_old */ = filecounter[newcount][tex] /* g_loaded_tex_new */;    // we need to reset the couter casues issues later at byte size calculation
                    filecounter[oldcount][vid] /* g_loaded_vid_old */ = filecounter[newcount][vid] /* g_loaded_vid_new */;
                    }
                    m_Timer.MsDelay(500);
...
/*
                            ...
                            gfx_init_textures(&state, g_loaded_tex_old, g_loaded_tex_new);  // outcommentend for the test of the h264 decoder to texture mechanism
*/
                    gfx_init_fshaders(&state, filecounter[oldcount][fsh] /* g_loaded_fsh_old */, filecounter[newcount][fsh] /* g_loaded_fsh_new */);
                    gfx_init_programs(&state, filecounter[oldcount][fsh] /* g_loaded_fsh_old */, filecounter[newcount][fsh] /* g_loaded_fsh_new */);
                    gfx_init_uniforms(&state, filecounter[oldcount][fsh] /* g_loaded_fsh_old */, filecounter[newcount][fsh] /* g_loaded_fsh_new */);
                    gfx_init_textures(&state, filecounter[oldcount][tex] /* g_loaded_tex_old */, filecounter[oldcount][tex] /* g_loaded_tex_new */);
...
if (m_H264Decoder.m_CharIndex >= 16384 && m_runtimelog == false )
                    ...
                    g_current_gl_program = util_choose_program();
                    ...
                    switch (g_centralModeBuffer[FRM_MODE][g_currentProgramBuffer])
                        {
