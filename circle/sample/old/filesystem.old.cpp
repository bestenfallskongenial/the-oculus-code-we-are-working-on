...
bool            CKernel::filesystem_mount           (   const char*         p_deviceName, 
                                                        ...
                                                        char*               fileNamesOmf[],          // <- new, we need to pass a single name and skip somehow the scanfile mechanism
                                                        ...
                                                        char*               fileNamesOmt[],       // <- same here, means also that we need the correct variables/arrays in global.h/cpp !
                                                        ...
                                                        int                 maxVideoFiles)
{
                ...
                if (f_partitionName != 0 && (m_pFileSystem = new CFATFileSystem) != 0 && m_pFileSystem->Mount(f_partitionName))
                    {
                    filecounter[scanendcount][vsh] /* g_scanned_vsh */ = filesystem_ScanRootDir(fileNamesVsh, g_SufVsh, SUFFIX_VSH, maxVshaderFiles);
                    filecounter[scanendcount][omf] /* g_scanned_fsh */ = filesystem_ScanRootDir(fileNamesOmf, g_SufOmf, SUFFIX_OMF, maxFoverlayFiles);                
                    filecounter[scanendcount][fsh] /* g_scanned_fsh */ = filesystem_ScanRootDir(fileNamesFsh, g_SufFsh, SUFFIX_FSH, maxFshaderFiles);
                    filecounter[scanendcount][tex] /* g_scanned_tex */ = filesystem_ScanRootDir(fileNamesOmt, g_SufOmt, SUFFIX_OMT, maxTexOverlayFiles);                
                    filecounter[scanendcount][tex] /* g_scanned_tex */ = filesystem_ScanRootDir(fileNamesTex, g_SufTex, SUFFIX_TEX, maxTextureFiles);
                    filecounter[scanendcount][vid] /* g_scanned_vid */ = filesystem_ScanRootDir(fileNamesVid,   g_SufVid, SUFFIX_VID, maxVideoFiles);

                    m_Watchdog.Start(8);
                    filecounter[newcount][vsh] /* g_loaded_vsh_new */ = filesystem_process_files(  fileNamesVsh, vStotalLoadedBytes, m_bufferVsh, 
                                                                filecounter[scannedcount][vsh]/* g_scanned_vsh */ , filecounter[newcount][vsh] /* g_loaded_vsh_new */, VSH_FILE_SIZE, 0);  // The file system was mounted successfully                 
                //  we need to know if we have done this correct - correct and all arrays and variables?  
                    m_Watchdog.Start(8);                                                                                
                    filecounter[newcount][omf]/* OMS_LOADED_NEW */ = filesystem_process_files(  fileNamesOmf, foverlayLoadedBytes, m_bufferOmf,          // <-the loader for the single overlay shader! - i could indeed use filesystem_load_file directly but... consistency, right?!
                                                                filecounter[scannedcount][omt]/* 1 or 0? */ , filecounter[newcount][omf] /* OMS_LOADED_NEW */ , FSH_FILE_SIZE, 1);                        // we need to define and assign the correct variables and arrays here and in global.* dont forget!
                    m_Watchdog.Start(8);        
                    filecounter[newcount][fsh]/* g_loaded_fsh_new */ = filesystem_process_files(  fileNamesFsh, fStotalLoadedBytes, m_bufferFsh, 
                                                                filecounter[scannedcount][fsh]/* g_scanned_fsh */ , filecounter[newcount][fsh] /* g_loaded_fsh_new */ , FSH_FILE_SIZE, 1);                           
                               
                    m_Watchdog.Start(8);    
                    filecounter[newcount][omt]/* OMT_LOADED_NEW */ = filesystem_process_files(  fileNamesOmt, tXoverlayLoadedBytes, m_BufferOmt,            // same shit, now for the overlay texture!!!
                                                                filecounter[scannedcount][omt]/* 1 or 0?*/, filecounter[newcount][omt] /* OMT_LOADED_NEW */ , TEX_FILE_SIZE, 2);                                                                                          
                    m_Watchdog.Start(8);    
                    filecounter[newcount][tex]/* g_loaded_tex_new */ = filesystem_process_files(  fileNamesTex, tXtotalLoadedBytes, m_bufferTex, 
                                                                filecounter[scannedcount][tex]/*g_scanned_tex */ , filecounter[newcount][tex] /* g_loaded_tex_new */ , TEX_FILE_SIZE, 2);                                   
                    m_Watchdog.Start(8);    
                    filecounter[newcount][vid]/* g_loaded_vid_new */ = filesystem_process_files(  fileNamesVid  , vItotalLoadedBytes, m_bufferVid  , 
                                                                filecounter[scannedcount][vid]/* g_scanned_vid */ , filecounter[newcount][vid] /* g_loaded_vid_new */ , VID_FILE_SIZE  , 3);                                
                    m_Watchdog.Start(8);    
