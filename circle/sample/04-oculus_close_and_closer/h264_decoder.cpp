...
bool            CKernel::MMALopenService          (   )
{
                ...
                tx_body.version.version_min                     = VC_MMAL_MIN_VER;
                tx_body.service_id                              = VCHIQ_MAKE_FOURCC('m','m','a','l');
                tx_body.connection                              = m_Connection;                                             // VCHI connection ( returned from bcmhost´s vc_host_get_vchi_state )
                tx_body.rx_fifo_size                            = 0;
                tx_body.tx_fifo_size                            = 0;
                tx_body.callback                                = MMALcallBack;
                tx_body.callback_param                          = &m_VCOSevent;
                tx_body.want_unaligned_bulk_rx                  = 0;
                tx_body.want_unaligned_bulk_tx                  = 0;
                tx_body.want_crc                                = 0;

                MMALstoreLog("\nversion                       ", tx_body.version.version);
                MMALstoreLog("version_min                   ", tx_body.version.version_min);
                MMALstoreLog("service_id                    ", tx_body.service_id);
                MMALstoreLog("connection                    ", (u32)(uintptr_t)tx_body.connection);
                MMALstoreLog("rx_fifo_size                  ", tx_body.rx_fifo_size);
                MMALstoreLog("tx_fifo_size                  ", tx_body.tx_fifo_size);
                MMALstoreLog("callback                      ", (u32)(uintptr_t)tx_body.callback);
                MMALstoreLog("callback_param                ", (u32)(uintptr_t)tx_body.callback_param);
                MMALstoreLog("want_unaligned_bulk_rx        ", tx_body.want_unaligned_bulk_rx);
                MMALstoreLog("want_unaligned_bulk_tx        ", tx_body.want_unaligned_bulk_tx);
                MMALstoreLog("want_crc                      ", tx_body.want_crc);

                int rc = vchi_service_open(m_VCHIInstance, &tx_body, &m_ServiceHandle);
                if ( rc != 0)
                    {
