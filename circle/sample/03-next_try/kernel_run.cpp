...
TShutdownMode   CKernel::Run(void)
{
                ...
                while (m_resetFlag == false)                                                // the actual mainloop - we loop while the reset flag is false
                    {
                    if(filesystem_update_USB("umsd1") == true && m_USBhasLoadOnes == false) // usb attach detection - do we need the flag when we manage to a) clean up the gl system and the filesystem properly ( soft reset )
                        {
