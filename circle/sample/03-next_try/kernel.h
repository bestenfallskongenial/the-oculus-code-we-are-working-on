...
// list of extensions used in my scanroot directory function per filetype 
        ...
        const   char                   *g_omfExtensions[FSH_VALID_SUFFIX_COUNT]			= { "omf" };	// is a fsh file but used for the overlay atlas
        const   char                   *g_fhsExtensions[FSH_VALID_SUFFIX_COUNT]			= { "fsh" };
        const   char                   *g_omtExtensions[TEX_VALID_SUFFIX_COUNT]			= { "omt" }; // is a bpm file but used for the overlay atlas
        const   char                   *g_texExtensions[TEX_VALID_SUFFIX_COUNT]			= { "bmp" };
        const   char                   *g_vidExtensions[VID_VALID_SUFFIX_COUNT]			= { "264" }; // i guess i will remove the whole parse code for anything but h264

// array to store the scanned filenames?
                char                   *g_vshScannedFileNames[VSH_FILES_ON_SD + VSH_FILES_ON_USB]     	= {   0    };
        		char				   *g_omfScannedFileNames[OMF_FILES_ON_SD + OMF_FILES_ON_USB] 		= {   0    };
                char                   *g_fshScannedFileNames[FSH_FILES_ON_SD + FSH_FILES_ON_USB]     	= {   0    };
        		char				   *g_omtScannedFileNames[OMT_FILES_ON_SD + OMT_FILES_ON_USB] 		= {   0    };
                char                   *g_texScannedFileNames[TEX_FILES_ON_SD + TEX_FILES_ON_USB]     	= {   0    };
                char                   *g_vidScannedFileNames[VID_FILES_ON_SD + VID_FILES_ON_USB]     	= {   0    };
// array to store the length of the loased files
                unsigned                g_vshLoadedBytes[VSH_FILES_ON_SD + VSH_FILES_ON_USB]          	= {   0    };
                unsigned                g_omfLoadedBytes[OMF_FILES_ON_SD + OMF_FILES_ON_USB]             	= {   0    };
                unsigned                g_fshLoadedBytes[FSH_FILES_ON_SD + FSH_FILES_ON_USB]          	= {   0    };
                unsigned                g_omtLoadedBytes[OMT_FILES_ON_SD + OMT_FILES_ON_USB]             	= {   0    };
